...
test('refuses to start native joining without a matching one-shot token and explicit profile', () => {
  assert.throws(() => buildServerConnectArguments({ serverAddress: '127.0.0.1:2001' }), /идентификатор/);
  assert.throws(() => buildServerConnectArguments({
...
test('rejects an unsafe direct server address', () => {
  assert.throws(() => buildLaunchArguments({
    mods: [],
...
test('recognizes the addon subfolder of the Workshop download directory', () => {
  assert.equal(isDownloadDirectory('B:\\Mods\\addons', 'B:\\Mods'), true);
  assert.equal(isDownloadDirectory('D:\\Mods', 'B:\\Mods'), false);
});
