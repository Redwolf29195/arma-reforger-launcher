...
```sh
sudo apt install ./LAR-Launcher-0.3.56-linux-x64.deb
```

The portable alternative is `LAR-Launcher-0.3.56-linux-x64.AppImage`; make it executable before opening it. AppImage needs FUSE 2 and a working Chromium sandbox. If Ubuntu/Mint blocks its sandbox, install the `.deb` instead; no system-wide sandbox weakening is needed. Linux updates are downloaded manually from the releases page; the Windows automatic updater remains separate.

Переносной вариант — `LAR-Launcher-0.3.56-linux-x64.AppImage`. Перед запуском разрешите выполнение файла. Для AppImage нужны FUSE 2 и доступная песочница Chromium. Если Ubuntu/Mint блокирует её, установите `.deb`. Обновления Linux скачиваются вручную со страницы релизов; автоматическое обновление Windows работает отдельно.
