...
A desktop launcher for presets, Workshop downloads and server connections.
Лаунчер для пресетов, загрузки модов Workshop и подключения к серверам.
...
```sh
sudo apt install ./LAR-Launcher-0.3.57-linux-x64.deb
```

The portable alternative is `LAR-Launcher-0.3.57-linux-x64.AppImage`; make it executable before opening it. AppImage needs FUSE 2 and a working Chromium sandbox. If Ubuntu/Mint blocks its sandbox, install the `.deb` instead; no system-wide sandbox weakening is needed. Linux updates are downloaded manually from the releases page; the Windows automatic updater remains separate.

Переносной вариант — `LAR-Launcher-0.3.57-linux-x64.AppImage`. Перед запуском разрешите выполнение файла. Для AppImage нужны FUSE 2 и доступная песочница Chromium. Если Ubuntu/Mint блокирует её, установите `.deb`. Обновления Linux скачиваются вручную со страницы релизов; автоматическое обновление Windows работает отдельно.
...
The display name changed to LAR Launcher in 0.3.48. The package ID, application ID and profile directory remain compatible with earlier versions; existing settings and presets keep their original paths. Historical release links and the GPL license are unchanged.

Version 0.3.49 applies the approved amber color `#F5A623` to the launcher artwork and application icons. Editable SVG masks retain the original transparent silhouette; PNG exports are used by the Windows and Linux packagers.
